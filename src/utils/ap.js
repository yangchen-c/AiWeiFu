  ...
  b.pay = function(d) {
    ...
    location.replace(window.location.origin+"/order/alipay?goto="+c)
      //####我的'#/alipay'是pay.htm页面改装过来的路由地址，需要自行替换
      //###################千万注意这个地方#################
      
  //由于微信浏览器 会在你带链接打开其他页面时截取掉 #后面的参数  所以需要一波处理
  // location.replace(window.location.protocol + "//" + window.location.host + "/?time=" + new Date() + "/order/alipay?goto=" + c)
